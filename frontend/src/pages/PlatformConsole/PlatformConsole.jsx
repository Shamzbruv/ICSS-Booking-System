...
function ContractsView() {
  ...
  const previewSigningExperience=async()=>{setMessage('');try{const blob=await api.platform.getAgreementTemplate();const url=URL.createObjectURL(blob);window.open(`/partner-contract.html?preview=1&document=${encodeURIComponent(url)}`,'_blank','noopener,noreferrer');setTimeout(()=>URL.revokeObjectURL(url),600000)}catch(err){setMessage(err.message)}};
