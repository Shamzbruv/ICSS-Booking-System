...
function ContractsView() {
  ...
  const countersign=async a=>{const body=forms[a.id]||{};if(!window.confirm(`Countersign ${a.partner_name||a.partner_email}'s agreement and activate their ${a.access_role==='developer_admin'?'developer admin':'partner'} account?`))return;setMessage('');try{const result=await api.platform.ownerSignAgreement(a.id,body);setMessage(result.message);load()}catch(err){setMessage(err.message)}};
  return <div className={s.view}><div className={s.view__header}><div><h2 className={s.view__title}>Contracts</h2><p className={s.chartHint}>Preview the agreement, track both signatures, complete your Owner fields, and download permanent copies.</p></div><button className={s.btnPrimary} onClick={previewTemplate}>Preview Contract Template</button></div>{message&&<div className={s.contractNotice}>{message}</div>}{loading?<div className={s.loading}>Loading contracts…</div>:<div className={s.contractGrid}>{agreements.map(a=><article className={s.contractCard} key={a.id}><div className={s.contractHead}><div><h3>{a.partner_name||a.partner_email}</h3><p>{a.partner_email}</p></div><span className={s.badge}>{a.status.replaceAll('_',' ')}</span></div><div className={s.contractFacts}><span><b>Access</b>{a.access_role==='developer_admin'?'Developer admin':'Marketing partner'}</span><span><b>Partner signed</b>{a.partner_signed_at?new Date(a.partner_signed_at).toLocaleString():'Waiting'}</span><span><b>Owner signed</b>{a.owner_signed_at?new Date(a.owner_signed_at).toLocaleString():'Waiting'}</span></div><div className={s.contractActions}><button className={s.btnGhost} onClick={()=>openPdf(a)}>Preview PDF</button><button className={s.btnGhost} onClick={()=>openPdf(a,true)}>Download copy</button></div>{a.status==='partner_signed'&&<div className={s.ownerFields}><h4>Owner completion fields</h4><input className={s.loginInput} type="date" aria-label="Effective date" onChange={e=>setField(a.id,'effectiveDate',e.target.value)}/><input className={s.loginInput} placeholder="Owner signature (typed full name)" onChange={e=>setField(a.id,'signature',e.target.value)}/><input className={s.loginInput} placeholder="Owner printed name" onChange={e=>setField(a.id,'printedName',e.target.value)}/><input className={s.loginInput} placeholder="Owner title / capacity" onChange={e=>setField(a.id,'title',e.target.value)}/><input className={s.loginInput} placeholder="Owner witness printed name" onChange={e=>setField(a.id,'witnessName',e.target.value)}/><input className={s.loginInput} placeholder="Owner witness signature" onChange={e=>setField(a.id,'witnessSignature',e.target.value)}/><button className={s.btnPrimary} onClick={()=>countersign(a)}>Countersign & Activate Account</button></div>}</article>)}{!agreements.length&&<div className={s.empty}>No contracts yet. Create a developer invitation from Developer Admins.</div>}</div>}</div>;
}
