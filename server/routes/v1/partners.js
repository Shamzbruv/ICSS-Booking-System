...
router.post('/invitations', authenticate, requirePlatformOwner, async (req, res) => {
...
router.post('/sign/:token', async (req, res) => {
    ...
    try {
        ...
        await sendPartnerAgreementEmail({ to: ownerEmail, subject: `Partner agreement ready for Owner signature — ${name}`, message: `${name} has signed the agreement. Sign in to the ICSS admin area and open Partner Contracts to complete the Owner fields.` });
...
router.get('/', authenticate, async (req, res) => {
    ...
    const result = isOwner(req)
        ? await query(`SELECT id,partner_email,partner_name,status,partner_signed_at,owner_signed_at,created_at FROM partner_agreements ORDER BY created_at DESC`)
        : await query(`SELECT id,partner_email,partner_name,status,partner_signed_at,owner_signed_at,created_at FROM partner_agreements WHERE partner_user_id=$1 ORDER BY created_at DESC`, [req.user.id]);
    res.json({ agreements: result.rows });
});

router.post('/:id/owner-sign', authenticate, requirePlatformOwner, async (req, res) => {
    ...
    try {
        ...
        const completed = await transaction(async client => {
            ...
            const userResult = currentUser.rows.length
                ? await client.query(`UPDATE users SET password_hash=$1,name=$2,role='platform_partner',active=true WHERE id=$3 RETURNING id`, [passwordHash,a.partner_name,currentUser.rows[0].id])
                : await client.query(`INSERT INTO users (email,password_hash,name,role,active) VALUES ($1,$2,$3,'platform_partner',true) RETURNING id`, [a.partner_email,passwordHash,a.partner_name]);
            const updated = await client.query(`UPDATE partner_agreements SET effective_date=$1,owner_signature=$2,owner_printed_name=$3,owner_title=$4,
                owner_witness_name=$5,owner_witness_signature=$6,owner_signed_at=NOW(),owner_user_id=$7,partner_user_id=$8,status='completed',updated_at=NOW() WHERE id=$9 RETURNING *`,
        ...
        await sendPartnerAgreementEmail({ to: completed.partner_email, subject: 'Your fully signed ICSS Partner Agreement and admin access', message: `The Owner has completed the agreement. Your final copy is attached and remains available in your Partner Contracts area. Admin login: ${baseUrl(req)}/admin/login.html\nEmail: ${completed.partner_email}\nTemporary password: ${password}\nPlease change this password using Forgot Password after signing in.`, pdfBuffer: pdf });
